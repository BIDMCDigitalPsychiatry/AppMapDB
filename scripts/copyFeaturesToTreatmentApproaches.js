...
const migrateDatabase = async () => {
  ...
  for (let i = 0; i < rowKeys.length; i++) {
    const k = rowKeys[i];
    const { features, treatmentApproaches, ...remaining } = rows[k];
    if (features !== undefined && features?.length > 0) {
      console.log('Migrating ' + k + ', i=' + i);      
      const valuesToCopy = ['ACT', 'CBT', 'DBT', 'iCBT or Sleep Therapy', 'Mindfulness', 'Physical Health Exercises'];
      const newTreatmentApproaches = [...features.filter(v => valuesToCopy.find(v2 => v2 === v))];
      if (newTreatmentApproaches.length > 0) {
        console.log({ newTreatmentApproaches });
      }
      // remove any un-nessary data and only keep the required fields to reduce snapshot size
      await updateRow({ ...remaining, features, treatmentApproaches: newTreatmentApproaches });
    } else {
