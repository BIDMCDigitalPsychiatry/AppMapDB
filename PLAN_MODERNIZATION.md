...
5. [x] `test@test.com` roster row deleted; `04_backfill_current_flags.js` audit run post-merge — **0 differences** across 1,819 flagged rows; temporary Chris-AppMap IAM access key deactivated (Chris) and the local `mindapps` AWS profile removed.
6. [x] Follow-up PR: gh-pages deploy action bumped `JamesIves@v3` → v4 (lowercase inputs, ambient token) — the last Node-deprecation action bump.
...
- **Lambda runtime upgrades**: `app-map-db-survey-reminders` rewritten on SDK v3 / nodejs20 (`cloud_functions/app-map-db-survey-reminders/`, deployed by `infrastructure/updateSurveyReminders.js`) — the old version authenticated as the PUBLIC Cognito unauth role in code, so the lockdown had broken it; it now uses its own scoped execution role. `app-map-db` (store-metadata proxy) bumped to nodejs20 config-only; both test-invoked live. (`CloudWatchImageAPI` and the Amplify deploy-time helpers are out of scope per Chris — other projects / inert.)
- **React 18 + `@mui/styles` retirement** (merged as PR #132): React 18.3 with `createRoot`; the deprecated JSS engine removed — `src/styles/jss.tsx` adapts the familiar makeStyles/withStyles API onto `tss-react`/emotion (the engine MUI v5 uses), 90 files import-codemodded, CssBaselineCustom rewritten on `GlobalStyles` (one styles object per `@font-face` — emotion collapses arrays under one key, which briefly dropped Lato; caught in the browser pass). Stepper pagination spacing fixed (fixed 320px width removed, 24px dot gutters).

**Still open (the only two):** Welltory merge decision (raters') · Dependabot triage (count jumped 28→158 when the first lockfile made transitives visible — visibility, not regression; needs a pass to separate runtime exposure from build-time noise).
